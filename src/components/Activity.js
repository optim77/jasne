...
function Activity() {
        ...
        try {
            ...
            if (res.ok) {
                res.json().then(data => {
                    ...
                    setPageCommentsCount(data.number)
                    console.log(data)
                })
            }
        } catch (e) {
    ...
    const handleNewsPageChange = ({selected}) => {
        fetchNewsActivity(selected);
    };
    const handleCommentsPageChange = ({selected}) => {
        fetchCommentsActivity(selected);
    };
                                        ...
                                        {comments['content'] && comments['content'].length > 0 ? comments['content'].map(content => (
                                            <div className="container mt-3">
                                                <div className="row">
                                                    ...
                                                    <button onClick={() => handleDeleteComments(content.id)} className="btn btn-danger m-1 col-1">Delete</button>
